(() => {
  ...
  const refreshBalance = () => {
    if (window.YamiWallet?.get) balance.textContent = window.YamiWallet.format(window.YamiWallet.get());
  };
  ...
  const injectWhiteLabel = () => {
    ...
    if (!doc.querySelector('script[src*="yami-wallet.js"]')) {
      ...
      doc.body.appendChild(wallet);
    }
    if (!doc.querySelector('script[src*="yami-shell.js"]')) {
      const shell = doc.createElement('script');
  ...
  frame.addEventListener('load', () => window.setTimeout(injectWhiteLabel, 30));
